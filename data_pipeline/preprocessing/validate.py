...
class DataValidator:
    ...
    def validate_population(self, gdf: gpd.GeoDataFrame) -> ValidationResult:
        ...
        if 'population' in gdf.columns:
            ...
            zero_pop = gdf[gdf['population'] == 0]
            if len(zero_pop) > 0:
                warnings.append(f"{len(zero_pop)} rows have zero population")
        else:
            errors.append("Missing 'population' column")
        ...
        # CRS check
        if gdf.crs is None:
            warnings.append("No CRS defined — assuming EPSG:4326")
        elif gdf.crs.to_epsg() != 4326:
            warnings.append(f"CRS is {gdf.crs.to_epsg()}, expected 4326")

        stats = {
            "total_points": len(gdf),
            "missing_geometry": len(missing_coords),
            "duplicates": int(duplicates.sum()),
            "total_population": int(gdf['population'].sum()) if 'population' in gdf.columns else 0,
        }
    ...
    def validate_buildings(self, gdf: gpd.GeoDataFrame) -> ValidationResult:
        ...
        try:
            ...
            zero_area = gdf[gdf_metric.area == 0]
        except Exception:
        ...
        if 'height_m' in gdf.columns:
            missing_height = gdf['height_m'].isna().sum()
            if missing_height > 0:
                warnings.append(f"{missing_height} buildings missing height")
        else:
            warnings.append("No 'height_m' column found")
        ...
        stats = {
            ...
            "invalid_geometry": int(invalid_geom.sum()),
    ...
    # ✅ FIXED: was outside the class before (indentation bug)
